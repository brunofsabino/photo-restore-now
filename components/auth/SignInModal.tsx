...
export function SignInModal({ isOpen, onClose, callbackUrl, packageId }: SignInModalProps) {
  ...
  const handleGuestCheckout = async (e: React.FormEvent) => {
    ...
    try {
      // Get a server-signed token — proves the user completed the guest form
      const tokenRes = await fetch('/api/guest-token', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: sanitizedEmail, name: sanitizedName }),
      });
      if (!tokenRes.ok) throw new Error('Failed to create guest session');
      const { token } = await tokenRes.json();

      // Keep sessionStorage for client-side auth check (email display etc.)
      const guestData = { email: sanitizedEmail, name: sanitizedName || 'Guest', timestamp: Date.now() };
      sessionStorage.setItem('guestCheckout', JSON.stringify(guestData));

      // Store signed token in cookie for server-side verification (4-hour expiry)
      document.cookie = `guestCheckout=${encodeURIComponent(token)}; path=/; max-age=14400`;
